...
if( !empty($_POST['btn_submit']) ) {
	...
	try {
		// SQL作成
		$stmt = $pdo->prepare("INSERT INTO english (view_name, message, post_date) VALUES ( :view_name, :message, :current_date)");
...
if( !empty($pdo) ) {
	...
	// メッセージのデータを取得する
	$sql = "SELECT view_name,message,post_date FROM english ORDER BY post_date DESC";
	...
	try {
		...
		if($search_word==""){
		  //echo "input search word";
			...
			$sql ="select * from english where message like '%".$search_word."%'";
			$sth = $dbh->prepare($sql);
			$sth->execute();
			$result = $sth->fetchAll();
		}
